...
with tab2: # 감정분석 통계
    ...
    dff = pd.read_excel('week_df_3_true.xlsx').drop(['Unnamed: 0'], axis = 1)
    dfff = pd.read_excel('week_df_3_total.xlsx')
    bin_intervals = pd.to_datetime(['2023-06-05 02:59:59', '2023-06-05 05:59:59', '2023-06-05 08:59:59',
                               '2023-06-05 11:59:59', '2023-06-05 14:59:59', '2023-06-05 17:59:59',
                               '2023-06-05 20:59:59', '2023-06-05 23:59:59'])
    bin_intervals = pd.to_numeric(bin_intervals)
    
    empty1,con1,empty2 = st.columns([0.01, 1.0, 0.01])
    empyt1,con2,empty2 = st.columns([0.01, 0.5, 0.01])
    empyt1,con3,empty2 = st.columns([0.01, 0.5, 0.01])
    ...
    with con1:
        ...
        tab2_selectbox_11 = st.selectbox('사용자 선택', dff['Datetime'].unique(), key = 'tab2 사용자 선택 11')
        
        selected_data = dff[(dff['User'] == tab2_selectbox_1) & (dff['Datetime'] == tab2_selectbox_11)]
        
        bin_intervals = pd.date_range('2023-06-05 02:59:59', '2023-06-05 23:59:59', freq='3H')
    
        chart_1 = alt.Chart(selected_data).mark_bar().encode(x = alt.X('End_Time:T', bin = alt.Bin(extent=[bin_intervals[0],
                                                                                                            bin_intervals[-1]]), title='Time'),
                 scale = alt.Scale(domain = bin_intervals.tolist()), y = alt.Y2('Negative_Count',
                            scale = alt.Scale(type = 'linear'))).properties(title = '일간 시간대별 부정 횟수 누적 그래프')
        
        text = alt.Chart(selected_data).mark_text(dx = 0, dy = 0, color = 'black').encode(x = 'End_Time:T',
                                        y = 'Negative_Count', detail = 'Start_Time', text = alt.Text('Negative_Count:Q'))
        
        st.altair_chart(chart_1 + text, use_container_width=True)
        
    with con2:
        tab2_selectbox_2 = st.selectbox('사용자 선택', dfff['User'].unique(), key = 'tab2 사용자 선택 2')
        dfff_Result = dfff.loc[dfff['User'] == tab2_selectbox_2].reset_index(drop=True)
        
        chart_2 = px.bar( dfff_Result, x = 'Week_Start_Date', y = 'Total number of negative sentence predictions per week',
                title = '주간별 총 부정 횟수 집계 그래프', text = 'Total number of negative sentence predictions per week', 
                         color_discrete_sequence = px.colors.qualitative.Pastel2)
        chart_2.update_traces(texttemplate='%{text:.2s}', textposition='outside')
        chart_2.update_layout(uniformtext_minsize=8, uniformtext_mode='hide')
        
        st.plotly_chart(chart_2, use_container_width=True)
        
    with con3:
        
        if (dfff_Result['Total number of negative sentence predictions per week'][0] <= 100) & (dfff_Result['Week_Start_Date'][0] == pd.to_datetime('2023-06-05')):
            st.success('정상입니다')
            
        elif (dfff_Result['Total number of negative sentence predictions per week'][0] <= 130) & (dfff_Result['Week_Start_Date'][0] == pd.to_datetime('2023-06-05')):
            st.info('주의가 필요합니다')
            
        elif (dfff_Result['Total number of negative sentence predictions per week'][0] <= 150) & (dfff_Result['Week_Start_Date'][0] == pd.to_datetime('2023-06-05')):
