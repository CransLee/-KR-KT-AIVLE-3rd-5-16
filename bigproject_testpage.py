...
with tab2: # 감정분석 통계
    ...
    t1_min_datetime, t1_max_datetime = min(Emotion_Stat_Dataset['Datetime']), max(Emotion_Stat_Dataset['Datetime'])
    st.text(t1_min_datetime)
    with t2_col1_1:
        st.subheader('대상자 선택')
        tab2_selectbox = st.selectbox('대상자 선택', Emotion_Stat_Dataset['User'].unique(), key = 'tab2_대상자선택', label_visibility="collapsed")
        tab2_targetdate = st.date_input('대상 날짜 선택', min_value = t1_min_datetime, max_value = t1_max_datetime, key = 'tab2_대상날짜선택', label_visibility="collapsed").dt.date
        st.text(tab2_targetdate.type)
        st.text(t1_min_datetime.type)
        Emotion_Stat_Dataset_Search_Result_1 = Emotion_Stat_Dataset.loc[Emotion_Stat_Dataset['User'] == tab2_selectbox & Emotion_Stat_Dataset['Datetime'] >= t1_min_datetime.dt.date & Emotion_Stat_Dataset['Datetime'] <= tab2_targetdate.dt.date]
